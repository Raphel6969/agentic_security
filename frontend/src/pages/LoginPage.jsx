...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    else if (err === 'oauth_failed') setError('OAuth login failed. Please verify your OAuth client configuration.');
    ...
    <div style={styles.root}>
      ...
      <div style={styles.card}>
        ...
        <div style={styles.sectionHeader}>
          <span style={styles.sectionText}>1-Click Role Login (Fast Test)</span>
        </div>
        ...
        <div style={styles.roleGrid}>
          <button
            ...
            style={{ ...styles.roleBtn, borderColor: '#f59e0b', color: '#fcd34d', background: 'rgba(245,158,11,0.1)' }}
          >
            {loadingRole === 'admin' ? '...' : '👑 Admin (Saswat)'}
          </button>
          <button
            onClick={() => handleDemoLogin('developer')}
            disabled={!!loadingRole}
            style={{ ...styles.roleBtn, borderColor: '#10b981', color: '#6ee7b7', background: 'rgba(16,185,129,0.1)' }}
          >
            {loadingRole === 'developer' ? '...' : '💻 Developer'}
          </button>
          <button
            onClick={() => handleDemoLogin('intern')}
            disabled={!!loadingRole}
            style={{ ...styles.roleBtn, borderColor: '#94a3b8', color: '#cbd5e1', background: 'rgba(148,163,184,0.1)' }}
          >
            {loadingRole === 'intern' ? '...' : '🎓 Intern (Restricted)'}
          </button>
          <button
            onClick={() => handleDemoLogin('tech_lead')}
            disabled={!!loadingRole}
            style={{ ...styles.roleBtn, borderColor: '#6366f1', color: '#a5b4fc', background: 'rgba(99,102,241,0.1)' }}
          >
            {loadingRole === 'tech_lead' ? '...' : '🛡️ Tech Lead'}
          </button>
        </div>

        <div style={{ ...styles.sectionHeader, margin: '20px 0 14px' }}>
          <span style={styles.sectionText}>Or Sign in with OAuth</span>
        </div>
        ...
        <div style={styles.pillRow}>
          ...
          <span style={styles.pill}>🛡️ Stage 0 RBAC</span>
          <span style={styles.pill}>⚡ Real-time SSE</span>
        </div>
      </div>
...
const styles = {
  ...
  particle: { position: 'absolute', borderRadius: '50%', background: 'rgba(99,102,241,0.6)', boxShadow: '0 0 8px rgba(99,102,241,0.4)' },
  gridOverlay: {
  ...
  subtitle: { margin: '0 0 6px', fontSize: 12, color: '#6366f1', fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' },
  tagline: { margin: '0 0 20px', fontSize: 13, color: '#94a3b8' },
  errorBox: {
  ...
  roleBtn: {
    border: '1px solid', borderRadius: 10, padding: '10px 8px', fontSize: 12,
    fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s ease',
  },
  buttonGroup: { display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 20 },
  oauthBtn: {
  ...
  pill: {
    ...
    border: '1px solid rgba(99,102,241,0.2)', borderRadius: 20, padding: '3px 8px',
  },
};
